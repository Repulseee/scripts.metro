...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717KKL_Electric")
TRAIN_SYSTEM.MVM = 1
...
function TRAIN_SYSTEM:SolveAllInternalCircuits(Train,dT,firstIter)
    ...
    --Reverser
    Train:WriteTrainWire(911,Train.Pred1C.Value)

 
    S["7D"] = T[10]*Train.A48.Value
    -- end 
    S["7G"] = S["7D"]*KV["10-7G"] --10-A48-RV-7G --FIXME 34w ARS
    S["B3"] = B*Train.A44.Value*Train.Pred1C.Value
...
--]]
        ...
        if isKSD then
            ...
            Train.RV2:TriggerInput("Set",S["33Yu"]*S["33Yu0"]*Train.RV21.Value)
            ...
            Train.R1_5:TriggerInput("OpenTime",6)
            -- print('1111111')
            ...
            Train.RPB:TriggerInput("Set",S["7D"]*KV["D-D1"]*Train.A79.Value*Train.PB.Value+Train.A79.Value*Train.ARS.Value*(1-Train.UOS.Value))
            ...
            Train:WriteTrainWire(1,S["10AK"]*Train.R1_5.Value+KRU["1/3-ZM31"]*-10)
            Train:WriteTrainWire(2,S["U2"]*KV["U2-2"]+ARS["6"]*RC1+AVO["2"]*RC2+KRU["2/3-ZM31"]*-10*Train.U22.Value) --U2-KV-2 FIXME ARS? 
            ...
            Train.R1_5:TriggerInput("Set",S["33"]*Train.R1_51.Value)
            -- Train.R1_5:TriggerInput("OpenTime",6)
            Train.RVT:TriggerInput("Set",S["U2a"]*Train.RVT1.Value)
            Train.K25:TriggerInput("Set",S["U2a"]*(ARS["33Zh"]*RC1+(1-RC1))*(AVO["025"]*RC2+(1-RC2))*Train.K251.Value)
            Train.K6:TriggerInput("Set",S["10AK"]*Train.RVT.Value) --10AK-K6
        ...
        Train:WriteTrainWire(17,S["U4"]*Train.VozvratRP.Value*(AVO["17"]*RC2+(1-RC2))*Train.RPVozvrat1.Value)
        Train:WriteTrainWire(71,S["U4"]*Train.OtklBV.Value)
        Train:WriteTrainWire(24,Panel.LSN*Train.KSN.Value*Train.KSN1.Value)
        ...
        Train:WriteTrainWire(91, Train.A79.Value*Train.ARS.Value*Train.PB.Value+T[87]*Train.ARSR.Value*(Train.KVTR.Value+Train.PB.Value))
        ARS.VP = ARS.FMM1*Train.VP.Value
        Train:WriteTrainWire(88,ARS.FMM1*Train.ALSFreq.Value*(1-Train.VP.Value)) 
        ARS.PD = T[88]
        ...
        S["14G"] = S["7G"]*Train.A42.Value+S["14a"]*Train.A42.Value
        ARS.ARSPower = S["14G"]*Train.ARS.Value
        ARS.ALSPower = ARS.ALS
        Train.BLPM.Power = ARS.ALS
       
        ARS.NGPower = S["7D"]*RC1
        Train.BIS200.Power = T[10]*Train.A43.Value*(als_on+Train.ARS.Value)*Train.AIS.Value
        ARS.KB=T[91]*RC1+(ARS.ALS*(1-Train.BSM_GE.Value)+ARS.GE*Train.BSM_GE.Value)*Train.KVT.Value*Train.KVT1.Value

        Train.BSM_KRT:TriggerInput("Set",(max(0,T[6])+max(0,T[8]))*RC1)
        Train.BSM_KRH:TriggerInput("Set",(max(0,T[1])+T[14])*RC1)
        Train.BSM_KRO:TriggerInput("Set",(T[87]+S["7Ga"]*KV["7GA-RC27"]+S["14a"]*Train.A42.Value*(1-Train.KRP.Value))*(1-Train.BSM_KRH.Value)*Train.KRO1.Value)
        ...
        Train.ROT1:TriggerInput("Set",T[92]*Train.ROT11.Value)
        Train.ROT2:TriggerInput("Set",T[93]*Train.ROT22.Value)
        --ARSDEnd
        ...
        S["7Gb"] = ARS.FMM1*(Train.PB.Value+(Train.ARS.Value+T[91]+(T[87]+S["B3"]*Train.ARSR.Value)*(1-RC1))*(1-Train.UOS.Value))*Train.ARS1.Value
        ...
        S["U4"] = S["10AK"]*KV["10AK-U4"]*Train.A74.Value--10AK-KV-U4
        Train:WriteTrainWire(17,S["U4"]*Train.VozvratRP.Value*Train.RPVozvrat1.Value)
        Train:WriteTrainWire(71,S["U4"]*Train.OtklBV.Value*Train.OtklBV1.Value)
        ...
        local ASNP_VV = Train.ASNP_VV
        ASNP_VV.Power = Train.AS1.Value*Train.R_ASNPOn.Value
        ASNP_VV.AmplifierPower = ASNP_VV.Power*Train.ASNP.LineOut*Train.R_UNch.Value*Train.A26.Value
        Train:WriteTrainWire(13,ASNP_VV.AmplifierPower)
        Train:WriteTrainWire(-13,ASNP_VV.AmplifierPower*Train.PowerSupply.X2_2)
        ASNP_VV.CabinSpeakerPower = ASNP_VV.Power*Train.R_UNch.Value*Train.ASNP.LineOut*Train.R_G.Value
    ...
    if isMVM then
        ...
        -- Train.R1_5:TriggerInput("OpenTime",2)
        Train.K6:TriggerInput("OpenTime",0.4)
    else
        S["1A"] = T[1]*Train.A1.Value
    end
    S["6A"] = T[6]*Train.A6.Value
    Train.TR1:TriggerInput("Set",S["6A"]*Train.TR11.Value)
    ...
    Train.LK1:TriggerInput("Set",S["1Zh"]*P.PM*Train.LK11.Value)
    Train.LK3:TriggerInput("Set",S["1Zh"]*Train.LK31.Value)
    Train.LK4:TriggerInput("Set",S["1Zh"]*Train.LK3.Value*Train.LK41.Value)
    ...
    S["20K"] = S["20B"]*P.PS
    Train.LK2:TriggerInput("Set",S["20K"]*S["ZR"]*Train.LK22.Value)
    Train.LK5:TriggerInput("Set",S["20B"]*Train.LK1.Value*S["ZR"]*Train.LK51.Value)
    ...
    Train.RKR:TriggerInput("Set",(S["4A"]*Reverser.NZ+S["5A"]*Reverser.VP)*Train.BV.State*S["ZR"]*Train.RKR1.Value) --81-717.5 Харько*S["ZR"]в
    --+B
    ...
    P:TriggerInput("PS",S["5Zh"]*(P.PP)*Train.PS1.Value)
    P:TriggerInput("PM",S["5Zh"]*(1-Train.TR1.Value)*Train.KSH2.Value*Train.PM1.Value)
    P:TriggerInput("PT",S["5Zh"]*(P.PM)*(1-Train.KSH2.Value)*Train.PT1.Value)
    --P:TriggerInput("PP",S["5Zh"]*(P.PM))
    S["2A"] = T[2]*Train.A2.Value
    S["2T"] = S["2A"]*Train.TR1.Value
    Train.RSU:TriggerInput("Set",S["2T"]*Train.ThyristorBU5_6.Value*Train.RSU1.Value)     
    ...
    Train.RV1:TriggerInput("Set",S["2U"]*Train.RV11.Value)
    ...
    if isMVM then
        Train:WriteTrainWire(512,Train.Pred2C.Value) 
        Train:WriteTrainWire(914,Train.Pred3C.Value) 
        Train.BV:TriggerInput("Power",BO*Train.A80.Value*Train.Pred3C.Value) 
    ...
    Train:WriteTrainWire(10,BO*Train.A56.Value*Train.Pred2C.Value)
    --B->A44->KMMK->23
    Train:WriteTrainWire(23,S["B3"]*Train.RezMK.Value*Train.RezMK1.Value)
    ...
    elseif isLVZ then
        ...
        Train:WriteTrainWire(49,Train.A49.Value*Train.AVOSV.Value)
        Train:WriteTrainWire(555,Train.L_2.Value*BO)
        S["48A"] = T[10]*Train.AV1.Value*Train.VZ1.Value+ARS["48"]*RC1
    end
    if self.NoRT2 then
    ...
    if self.HaveRO then
        ...
        elseif isMVM then
            Train.RO:TriggerInput("Set",(T[48]+ARS["48"])*RC1*Train.RO1.Value)
        else
    ...
    S["22B"] = T[10]*Train.A10.Value*Train.VMK.Value*Train.MK1.Value
    ...
    S["F1"] = S["B9"]*KV["B9-F1"]
    Train:WriteTrainWire(72,BO*Train.A53.Value*S["F1"]*(1-T[5]))
    if isLVZ then
        if isKSD then
        ...
        Train.BZOS.Ring = (1-Train.SQ3.Value)*Train.SAB1.Value  
        Train:WriteTrainWire(7,S["UO"]*Train.Ring.Value+ARS.Ring+Train.BZOS.Ring*BO*(T[5]+T[4]))
    ...
    Panel.Ring = T[7]+(1-Train.VB.Value)*Train.A53.Value*(1-Train.ParkingBrake.Value)
    ...
    Train:WriteTrainWire(556,Train.RPU2.Value)
    Train:WriteTrainWire(69,S["36N"]*Train.BPSNon.Value*Train.BPSN1.Value)
    Train:WriteTrainWire(36,T[69]*(1-Train.BPSNon.Value)*Train.BPSN1.Value*BO)
    Panel.LKVP = T[36]
    Panel.RZP = T[36]*T[61]

    S["B9a"] = S["B9"]*Train.VB.Value 
    Train.KVC:TriggerInput("Set",S["B9a"]*Train.KVC1.Value)
    ...
    if isLVZ then
        ...
        Train:WriteTrainWire(16,S["D1"]*(Train.VUD1.Value*Train.VUD2.Value+ARS.GE*C(Train.ALSCoil.Speed>5)))
        S[31] = S["D1"]
        S[32] = S["D1"]
    end
...
--end
    ...
    --FIXME ARS FMM2
    --F1->F2->BPF
     S["F8"] = S["F7"]*Train.VUS.Value
     S["F999"] = S["F7"]*Train.L_4.Value
     S["F888"] = S["F7"]*Train.A1.Value
    Train.Panel.Headlights1 = S["F8"]*Train.VUS.Value*Train.A46.Value*Train.VUS1.Value
    Train.Panel.Headlights2 = S["F999"]*Train.L_4.Value*Train.A47.Value*Train.VUS1.Value
    Train.Panel.LPiz = S["F888"]*Train.A1.Value
    ...
    --Вспом цепи приём
    Panel.EmergencyLights = Train.A49.Value*Train.AVOSV.Value
    Train.RPU:TriggerInput("Set",Train.A37.Value*T[25])
    Train.Schemes = S

    S["D6"] = S["D4"]*Train.BD.Value
    Train.RD:TriggerInput("Set",S["D6"]*Train.RD1.Value)
    Panel.DoorsW = S["D4"]*(1-Train.RD.Value)
    Train.VDZ:TriggerInput("Set",T[16]*Train.A16.Value*(1-Train.RD.Value)*Train.VDZ1.Value)
    S["12A"] = T[12]*Train.A12.Value
    S["31A"] = T[31]*Train.A31.Value*Train.A311.Value
    S["32A"] = T[32]*Train.A32.Value*Train.A321.Value
    Train.VDOL:TriggerInput("Set",S["31A"]+S["12A"]*Train.VDOL1.Value)
    Train.VDOP:TriggerInput("Set",S["32A"]+S["12A"]+T[68]*Train.A68.Value*Train.VDOP1.Value)

    S["36A"] = T[36]*Train.A51.Value*Train.RVO.Value--36
    Train.KVP:TriggerInput("Set",S["36A"]*Train.KPP.Value*Train.KVP1.Value)
    Train.KPP:TriggerInput("Set",S["36A"]*(1-Train.RZP.Value)*Train.KVC.Value)
    Train:WriteTrainWire(913,Train.Pred4C.Value)
    S["27A"] = T[27]*Train.A50.Value*Train.Pred4C.Value                     
    Train.KO:TriggerInput("Set",S["27A"]*Train.KO1.Value)
    --S["22A"] = (T[23]*Train.A23.Value+T[22]*Train.A22.Value) --FIXME 714
    S["22A"] = T[22]*Train.A22.Value
    Train.KK:TriggerInput("Set",S["22A"]*(1-Train.TRK.Value)*Train.KK1.Value)  
    ...
    Train.RPU:TriggerInput("Set",T[556]*Train.A37.Value)
    Train.RZP:TriggerInput("Open",T[556]*Train.A37.Value*Train.RPU.Value)
...
function TRAIN_SYSTEM:SolveRKInternalCircuits(Train,dT,firstIter)
    ...
    local RheostatController = Train.RheostatController
    local RK    = RheostatController.SelectedPosition
    local B     = (Train.Battery.Voltage > 55) and 1 or 0*Train.Pred2C.Value
    local BO    = B*Train.VB.Value*Train.Pred2C.Value
    local T     = Train.SolverTemporaryVariables
    ...
    S["10R"] = S["10A"]*(1-Train.LK3.Value)*C(2<=RK and RK<=18)*(1-Train.LK4.Value)
    S["2U"] = S["10R"]+S["2C"]*S["ZR"]
    Train.SR1:TriggerInput("Set",S["2U"]*Train.SR11.Value)
    ...
    Train["RRTuderzh"] = S["25A"]
    Train["RUTpod"] = S["10A"]*RheostatController.RKM1*S["10X"]
    Train.RRT:TriggerInput("Close",Train.RRTuderzh*Train.RRTpod+T[25])
    Train.RRT:TriggerInput("Open",(1-Train.RRTuderzh)*(1-Train.RPU.Value))
...
local wires = {1,2,3,4,5,6,7,8,10,12,14,15,16,17,18,19,20,22,23,24,27,-28,28,25,13,-13,31,32,36,37,39,44,48,49,53,54,57,59,60,58,57,64,34,36,-34,61,64,66,67,68,69,71,72,87,88,91,92,93,98,99,555,556,911,512,913,914}

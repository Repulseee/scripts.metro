-- БАРС-М
Metrostroi.DefineSystem("BARS_M")
...
function TRAIN_SYSTEM:Initialize()
    ...
    self.Train:LoadSystem("BLPM","ALS_BARS_BLPM")
...
function TRAIN_SYSTEM:Think(dT)
    ...
    Train.FMM1:TriggerInput("Set",self.FMM1*Train.FMM11.Value)
    ...
    Train.BSM_GE:TriggerInput("Set",self.GE*Train.GE1.Value+(Train.ALSCoil.Speed>4 and 1 or 0))
    self.KT = Train.BSM_GE.Value*Train.BUM_RVZ1.Value
    -- LN 1/5 Light fix
    ...
    Train.NG:TriggerInput("Set",S["FMM_28"]*Train.FMM1.Value+self.NGPower*Train.NG.Value*(1-Train.NH.Value)+Train.VN.Value*Train.NG1.Value)
    ...
    S["SRSIR5"] = (S["NF"]*(1-Train.BSM_SIR5.Value)+S["5SIR"]*Train.BSM_SIR5.Value)*Train.SIR55.Value
    ...
    Train.BSM_SR1:TriggerInput("Set",S["SR"]*Train.SR11.Value)
    Train.BSM_SR2:TriggerInput("Set",S["SR"]*Train.SR11.Value)
    ...
    Train.BSM_KSR1:TriggerInput("Set",S["KSR"]*Train.KSR11.Value)
    Train.BSM_KSR2:TriggerInput("Set",S["KSR"])
 
    Train.BSM_BR1:TriggerInput("Set",self.KB*((Train.BSM_BR1.Value+Train.BSM_SIR5.Value)*(1-Train.BSM_BR2.Value))*Train.BR11.Value)
    Train.BSM_BR2:TriggerInput("Set",self.KB*((1-Train.BSM_SIR5.Value)*(1-Train.BSM_BR1.Value))*Train.BR11.Value)
    ...
    Train.BUM_RUVD:TriggerInput("Set",S["GEKSR"]*(Train.BSM_KRO.Value+Train.BUM_RUVD.Value)*Train.RUVD1.Value)
    ...
    Train.BUM_RVT5:TriggerInput("Set",self.GE*(1-Train.BUM_TR.Value)*Train.RVT11.Value)

    S["RVT1"] = self.GE*((1-Train.BSM_SIR4.Value)*(1-Train.BUM_TR.Value)+(1-Train.BUM_PTR1.Value))
    Train.BUM_RVT1:TriggerInput("Set",S["RVT1"]*Train.RVT11.Value)
    Train.BUM_RVT2:TriggerInput("Set",S["RVT1"]*Train.RVT11.Value)
    Train.BUM_RVT4:TriggerInput("Set",S["RVT1"]*Train.RVT11.Value)
    ...
    Train.BUM_RVZ1:TriggerInput("Set",S["RVZ1"]*(1-Train.BSM_BR2.Value)*Train.RVZ11.Value) 
    ...
    self.EPK = self.GE*Train.BUM_EK.Value*Train.BUM_EK1.Value*Train.EPK1.Value+(Train.ALSCoil.Speed>4 and 1 or 0)
    -- else 
        -- self.EPK = 1 
    -- end 
    ...
    self["48"] = self.GE*(Train.BUM_RET.Value+Train.BUM_RVZ1.Value)*(CurTime()-self.PNOffTimer > 0 and 1 or 0)*Train.VZ11.Value
    -- print('govno')
    ...
    -- print(bit.bor(ALS.F1*1,ALS.F2*2,ALS.F3*4,ALS.F4*8,ALS.F5*16,ALS.F6*32))
    ...
    Train:WriteTrainWire(86,self.GE*self.KT)
    --LN 1/5 Original
    ...
    --LN 1/5 Fix
    Panel.LN = Train.NG.Value*Train.PD1.Value+Train:ReadTrainWire(89)*Train.FMM1.Value

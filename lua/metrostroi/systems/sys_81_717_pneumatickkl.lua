...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717KKL_Pneumatic")
...
function TRAIN_SYSTEM:Think(dT)
    ...
    if Train.AirDistributorDisconnect.Value == 0 then
        ...
        if Train.PneumaticNo2.Value == 1.0 then
            if self.PN2 == 0 then
                self.PN2 = math.min(self.TrainLinePressure,(2.4 + error + self.WeightLoadRatio*1.3))
        ...
        if self.BrakeCylinderValve == 1 then
            self:equalizePressure(dT,"BrakeCylinderPressure", math.min(2.5 + self.WeightLoadRatio*1.3,targetPressure), 1+math.Clamp((self.BrakeCylinderPressure-0.5)/2.8,0,0.7), 3.50, nil, 0.8+math.Clamp((self.BrakeCylinderPressure-0.75)/0.6,0,1)) --0.75, 1.25)
        end
    ...
    Train.BPT:TriggerInput("OpenTime", math.Rand(1,2.35))
    ...
    self.LeftDoorSpeed[1] = self.LeftDoorSpeed[2]+0.2
    self.RightDoorSpeed[1] = self.LeftDoorSpeed[2]+0.2

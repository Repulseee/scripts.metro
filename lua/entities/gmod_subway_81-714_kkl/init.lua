...
ENT.SyncTable = {
    ...
    "AV3","AV4","AV5","A81","AV6","A80","A18","Pred1C","Pred2C","Pred3C","Pred4C", 
    "VB","GV",
...
function ENT:Initialize()
    ...
    self:SetModel("models/metrostroi_train/81-717/base_714_int.mdl")
...
function ENT:TrainSpawnerUpdate()
    ...
    if self.CustomSettings then
        ...
        local typ = self:GetNW2Int("BodyType",0)
     ...
     elseif interor == 2 then
     ...
     end--   self:SetNW2String("PassTexture",tex)
        ...
        self:SetNW2String("Texture","Def_717KKLKYIVClassic")
...
function ENT:Think()
    ...
    -- self.BPT:TriggerInput("OpenTime", math.Rand(1,2.35))
    ...
    self:SetPackedBool("Pred1C",self.Pred1C)  
    self:SetPackedBool("Pred2C",self.Pred2C)
    self:SetPackedBool("Pred3C",self.Pred3C) 
    self:SetPackedBool("Pred4C",self.Pred4C)  
  
     
    self:SetPackedBool("CouchCap",self.CouchCap)
    self:SetPackedBool("CouchCap2",self.CouchCap2)
    ...
    if IsValid(self.FrontBogey) and IsValid(self.RearBogey) and not self.IgnoreEngine then
        ...
        self.FrontBogey.MotorForce = 27000+5500*(A < 0 and 1 or 0)
        self.RearBogey.MotorForce  = 27000+5500*(A < 0 and 1 or 0)
...
function ENT:OnButtonPress(button,ply)
    ...
    if button == "Pred4" then self.Pred4 = not self.Pred4  end
     ...
     if button == "Pred1CToggle" then self.Pred1C = not self.Pred1C end 
      if button == "Pred2CToggle" then self.Pred2C = not self.Pred2C end 
      if button == "Pred3CToggle" then self.Pred3C = not self.Pred3C end 
      if button == "Pred4CToggle" then self.Pred4C = not self.Pred4C end
